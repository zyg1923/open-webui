...
def drop_failed_assistant_turns(messages: list[dict]) -> list[dict]:
    """Drop empty/error assistant turns so the same chat can continue after a failed reply.

    Qwen/OpenAI-style gateways reject empty assistant content. Old chats that already
    have a blank bubble must still be usable without opening a new conversation.
    """
    kept: list[dict] = []
    for message in messages:
        ...
        if role == 'assistant':
            ...
            if (has_error or not text) and not has_tools and not has_output:
                continue
        kept.append(message)

    repaired: list[dict] = []
    for message in kept:
        if repaired and repaired[-1].get('role') == 'user' and message.get('role') == 'user':
            repaired.append({'role': 'assistant', 'content': '（上一次回复失败，已跳过）'})
...
def normalize_messages_for_model(form_data: dict) -> dict:
    ...
    form_data['messages'] = drop_failed_assistant_turns(form_data.get('messages', []))
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    form_data['messages'] = drop_failed_assistant_turns(form_data['messages'])
    # Old chats may still send native tools / extra features; drop them so this
    # turn can continue in the same conversation after a previous failure.
    form_data.pop('tools', None)
    form_data.pop('functions', None)
    ...
    # Old chat UI state keeps these flags; they make Qwen return empty and then
    # the user can only start a new chat. Force them off for every turn.
